...
function install_package(){
	...
	foreach($ctypes as $ctype){
        ...
        foreach ($varchar_fields as $varchar_field) {
            ...
            $core->db->query("ALTER TABLE  `{#}{$content_model->table_prefix}{$ctype['name']}` CHANGE  `{$varchar_field}`  `{$varchar_field}` VARCHAR( 256 ) NULL DEFAULT NULL;", false, true);

            $core->db->query("ALTER TABLE  `{#}{$content_model->table_prefix}{$ctype['name']}_cats` CHANGE  `{$varchar_field}`  `{$varchar_field}` VARCHAR( 256 ) NULL DEFAULT NULL;", false, true);
        }
    ...
    if(!$core->db->isFieldExists('content_datasets', 'index')){
        $core->db->query("ALTER TABLE `{#}content_datasets` ADD `index` VARCHAR(40) NULL DEFAULT NULL COMMENT 'Назва індексу, який використовується' AFTER `sorting`, ADD INDEX (`index`);");
    }

    if(!$core->db->isFieldExists('controllers', 'is_external')){
        $core->db->query("ALTER TABLE `{#}controllers` ADD `is_external` TINYINT(1) UNSIGNED NULL DEFAULT NULL COMMENT 'Сторонній компонент' AFTER `is_backend`");
    }

    if(!$core->db->isFieldExists('rss_feeds', 'template')){
        $core->db->query("ALTER TABLE  `{#}rss_feeds` ADD  `template` VARCHAR(30) NOT NULL DEFAULT  'feed' COMMENT  'Шаблон стрічки';");
    }
    ...
    if(!$config->save($values)){
        cmsUser::addSessionMessage('Не можу записати файл конфігурації сайту. Додайте до нього рядок <b>"db_engine" => "InnoDB",</b>. Після цього зробіть цей файл доступним для запису, зайдіть в загальні налаштування сайту і просто перезбережіть їх.', 'info');
    }
